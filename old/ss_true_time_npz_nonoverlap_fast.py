...
def loglog_secants(x, y):
    """Calculates the log-log slopes using method of secants.
    Given a collection of points (x_1, y_1) ... (x_n, y_n), this
    function returns (x_i, s_i) where s_i is the log-log slope fitted to
    all points before (x_j, y_j) , j <= i.
    """
    y_dim = len(y)
    s = np.zeros(y_dim)
    ...
    for i in range(y_dim):
        if i == 0:
            continue
        s[i] = fit_loglog_slope(x[:i+1], y[:i+1])
    return x[1:], s[1:] 
